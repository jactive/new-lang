...
fn test_parse_complex() {
    ...
    assert_eq!(parse_complex("1.3,"), None);
    assert_eq!(
...
fn main2() {
    ...
    if args.len() != 5 {
        eprintln!("Usage: {} FILE PIXLS UPPER-LEFT LOWER-RIGHT", args[0]);
        eprintln!(
            "Example: {} mandel.png 1000x750 -1.20,0.34 -1,0.20",
            args[0]
        );
        std::process::exit(1);
    }

    let bounds = parse_pair::<usize>(&args[2], 'x').expect("error parsing image dimesions");
